# R Script for visualising heatmap of gene expression
...
# 4. Extract Entrez Gene IDs from the row names of the log-transformed data
ncbi_gene_ids <- rownames(log_fpkm_data)

# 5. Query biomaRt to get Gene Symbols using NCBI Gene IDs
gene_annotations <- getBM(attributes = c('entrezgene_id', 'hgnc_symbol'),
                          filters = 'entrezgene_id',
                          values = ncbi_gene_ids,
                          mart = ensembl)

# 6. Merge log-transformed FPKM data with gene_annotations
...
# 7. Remove rows with missing or empty gene symbols
log_fpkm_data <- log_fpkm_data[!is.na(log_fpkm_data$hgnc_symbol) & log_fpkm_data$hgnc_symbol != "", ]

# 8. Ensure no numeric-only gene symbols are present and remove them
log_fpkm_data <- log_fpkm_data[!grepl("^[0-9]+$", log_fpkm_data$hgnc_symbol), ]

# 9. Replace row names with unique gene symbols
...
# 10. Select top 10% most variable genes based on standard deviation
...
# 11. Subset data to include only the top 10% most variable genes
